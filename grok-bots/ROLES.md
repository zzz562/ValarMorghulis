...
**每个 bot 的档案栏只贴它自己那一段（`roles/<bot>.md`）**,不要六段都塞进同一个 bot——上下文会互相污染,分轨就废了。**`roles/whale-trail-lab.md`（群说明）只贴进 whale-trail-lab 群**,不进任何单个 bot。
...
valar 远端 (origin/main)  ──读取──▶  box(bot 群 whale-trail-lab)
   ...
   ├─ grok-bots/roles/whale-trail-lab.md  群协作说明（只贴同名群）
   ├─ journal/                       跨项目决策与工作日志
...
| — | **whale-trail-lab**（群说明） | 群协作规则与发言顺序（只贴该群，非 bot） | [`roles/whale-trail-lab.md`](roles/whale-trail-lab.md) |
